(function () {
  ...
  async function listActiveNegatives(mode, limit = 20) {
    const all = await root.Dumly.db.getAllByIndex('negativeMemories', 'mode', mode);
    const now = Date.now();
    return all
  ...
  function retentionScore(m) {
    ...
    return s.recency(m.createdAt) * s.usageBoost(m) * strength * s.editBoost(m);
  }

  async function runCleanup() {
    const now = Date.now();

    const cands = await root.Dumly.db.getAll('suggestionCandidates');
    for (const c of cands) {
      if (c.expiresAt < now) await root.Dumly.db.del('suggestionCandidates', c.id);
    }

    const negs = await root.Dumly.db.getAll('negativeMemories');
    for (const n of negs) {
      if (n.expiresAt < now) await root.Dumly.db.del('negativeMemories', n.id);
    }

    const sessions = await root.Dumly.db.getAll('generationSessions');
    const sessionCutoff = now - LIMITS.sessionTTLHours * 60 * 60 * 1000;
    for (const s of sessions) {
      if (s.updatedAt < sessionCutoff) await root.Dumly.db.del('generationSessions', s.id);
    }

    const ins = await root.Dumly.db.getAll('insertionRecords');
    const insCutoff = now - LIMITS.insertionRecordTTLDays * DAY_MS;
    for (const r of ins) {
      if (r.insertedAt < insCutoff) await root.Dumly.db.del('insertionRecords', r.id);
    }

    const accepted = await root.Dumly.db.getAll('acceptedMemories');
    ...
    const negsAfter = await root.Dumly.db.getAll('negativeMemories');
