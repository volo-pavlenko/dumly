(function () {
  ...
  function buildCardDom(mode) {
    ...
    // Topic row
    const topicBlock = el('div', { className: 'dumly-card-topic' });
    const topicInput = el('input', {
      className: 'dumly-card-topic-input',
      attrs: {
        type: 'text',
        placeholder: 'Optional topic or focus',
        autocomplete: 'off',
        maxlength: '200',
        'aria-label': 'Optional topic or focus',
      },
    });
    topicBlock.append(topicInput);
    ...
    const regenLabel = el('span', { text: 'Regenerate' });
    regenBtn.append(regenerateIcon(), regenLabel);
    ...
    root.append(header, subheader, topicBlock, bodyWrap, actions, toneBlock, usingBlock, footer);
    return {
      root, body, topicInput, overflowMenu, overflowTrigger,
      useBtn, regenBtn, regenLabel, toneRow, usingRow, footer,
  ...
  function mount(editorContainer, anchorBtn, initial, handlers) {
    ...
    mounted = {
      root,
      body: dom.body,
      topicInput: dom.topicInput,
      useBtn,
      regenBtn,
      regenLabel: dom.regenLabel,
      overflowTrigger,
      toneRow,
      onKey,
      onOutside,
    ...
    return {
      ...
      getTopic: () => mounted?.topicInput?.value?.trim() || '',
      getTone: () => mounted?.toneRow?.querySelector('.is-active')?.getAttribute('data-tone') || 'default',
    };
  }

  function updateActionState({ loading = false } = {}) {
    if (!mounted) return;
    const hasSuggestion = !!mounted.currentCandidateId;
    mounted.root.classList.toggle('has-suggestion', hasSuggestion);
    mounted.root.classList.toggle('is-generating', loading);
    mounted.useBtn.disabled = loading || !hasSuggestion;
    mounted.regenBtn.disabled = loading;
    mounted.overflowTrigger.disabled = loading || !hasSuggestion;
    mounted.regenLabel.textContent = hasSuggestion ? 'Regenerate' : 'Generate';
    mounted.toneRow.querySelectorAll('[data-tone]').forEach((btn) => {
  ...
  function setState(state, message) {
    ...
    if (state === 'ready') {
      mounted.currentCandidateId = null;
      body.textContent = 'Choose a tone or focus, then generate.';
      updateActionState();
    } else if (state === 'loading') {
      body.classList.add('is-loading');
      body.textContent = 'Generating...';
      updateActionState({ loading: true });
    } else if (state === 'error') {
